...
class _AdditionalBuildings(SC2Action):
    ...
    def _barracksReady(self, obs):
        ...
        else:self._act = RallyPoint(units.Terran.Barracks, ([54,42] if self._top else [28,20]))
    ...
    def action(self, obs):
        ...
        if self._act.isFinished():
            ...
            elif self._iteration == 4:self._act = Builder(BFUNCID.MISSILETURRET, [42,48] if self._top else [46,37])
