...
class _AdditionalBuildings(SC2Action):
    ...
    def action(self, obs):
        ...
        if self._act.isFinished():
            ...
            elif self._iteration ==8:self._act = RallyPoint(units.Terran.Barracks, ([52, 48] if self._top else [28, 20]))
            ...
            elif self._iteration == 12:self._act = MoveCamera(self._initial_camera_position)
            elif self._iteration == 13:self._act = Harvest(units.Neutral.MineralField, queued=True)
            elif self._iteration == 14:self._act = MoveCamera([28.5, 23.5] if self._top else [30.5, 48.5])
